...
const Vertex* Object2D::vertex = NULL;
const GLubyte* Object2D::index = NULL;

void Object2D::createVao() {
    GLuint vboId[2];
...
void Object2D::draw(glm::vec4 rgba, glm::mat4 matrix, GLint MatrixId, GLint ColorId) {
    size_t length = sizeof(this->vertex) / sizeof(this->vertex[0]);
    glBindVertexArray(this->vao);
    glUniformMatrix4fv(MatrixId, 1, GL_FALSE, glm::value_ptr(matrix));
    glUniform4fv(ColorId, 1, glm::value_ptr(rgba));
    glDrawElements(GL_TRIANGLES, length, GL_UNSIGNED_BYTE,
        reinterpret_cast<GLvoid*>(0));
}
